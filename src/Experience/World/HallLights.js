...
export default class HallLights {
  ...
  setCeilingSpotlights() {
    ...
    for (let i = 0; i < totalCeilings; i++) {
      ...
      // Downlights along the lane area
      for (let j = 3; j < 10; j++) {
        const zPos = 180 - j * 40;
      ...
      // Back wall wash
      const backWallWash = new THREE.SpotLight(
        0xffffff,
        50,
        100,
        Math.PI / 3,
  ...
  setFrontGridLights() {
    ...
    for (let i = 0; i < totalGridX; i++) {
      ...
      for (let j = 0; j < totalGridZ; j++) {
        ...
        const gridSpot = new THREE.SpotLight(
          0xffffff,
          120,
          160,
          Math.PI / 6,
  ...
  setWallPointLights() {
    ...
    // Ceiling panels: alternating cyan/magenta for a stylised look
    for (let i = 0; i < totalCeilings; i++) {
      const xPos = startX + i * spacingX;
    ...
    // Side wall lights (pillar accents)
