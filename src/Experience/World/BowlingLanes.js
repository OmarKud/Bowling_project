...
export default class BowlingLanes {
  ...
  setMaterials() {
    this.laneTexture = this.textureLoader.load(
  ...
  buildLanes() {
    ...
    this.gutterGeometry = new THREE.BoxGeometry(
      gutterWidth,
      0.1,
      this.laneLength,
    );

    this.dividerGeometry = new THREE.CylinderGeometry(
      cappingRadius,
      cappingRadius,
      this.laneLength,
      64,
    ...
    for (let i = 0; i < this.totalLanes; i++) {
      ...
      leftGutter.position.set(
        xPos - laneWidth / 2 - gutterWidth / 2,
        0.22,
        this.centerZ,
      );
      ...
      if (i < this.totalLanes - 1) {
        ...
        const laneNeon = new THREE.Mesh(
          this.laneNeonGeometry,
          this.laneNeonMaterial,
        );
    ...
    const rightOuterX =
      startX +
      this.totalLanes * this.laneComponentWidth -
      this.laneComponentWidth / 2;
    const outerPositions = [leftOuterX, rightOuterX];

    for (let i = 0; i < outerPositions.length; i++) {
      const outerDividerMesh = new THREE.Mesh(
        this.dividerGeometry,
        this.dividerMaterial,
      );
      ...
      const outerLaneNeon = new THREE.Mesh(
        this.laneNeonGeometry,
        this.laneNeonMaterial,
      );
